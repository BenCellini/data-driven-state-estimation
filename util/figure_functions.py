...
class Colormaps:
    ...
    def __init__(self, color_dict=None, power=5, flip_linear=False, flip_log=True):
        ...
        if color_dict is None:
            ...
            cmap_z = mpl.colormaps['inferno'](np.linspace(0.0, 1.0, num=1000))
            cmap_z = np.vstack((cmap_z, np.matlib.repmat(cmap_z[-1], stack, 1))).tolist()

            self.color_dict = {'zeta': cmap_zeta, 'w': cmap_w, 'z': cmap_z}

            self.color_list = {'zeta': np.array([68.0, 21.0, 185.0]) / 255,
                               'w': np.array([22.0, 125.0, 8.0]) / 255,
                               'z': 'red',
                               }
        elif color_dict == 'inferno':
            res = np.linspace(0.0, 1.0, num=1000)
            self.color_dict = {'zeta': mpl.colormaps['inferno'](res).tolist(),
                               'w': mpl.colormaps['inferno'](res).tolist(),
                               'z': mpl.colormaps['inferno'](res).tolist()}

            # self.color_dict = {'zeta': mpl.colormaps['Blues'](res).tolist(),
            #                    'w': mpl.colormaps['Reds'](res).tolist(),
            #                    'z': mpl.colormaps['Greens'](res).tolist()}
...
def plot_heat_map_squares(heatmap_values, cmap=plt.colormaps['inferno_r'], cnorm=None, ax=None,
                          alpha=0.1, labels=None, right_labels=None, cmap_ticks=None, abs=False):
    """ Plot heatmap as individual square patches.
    """

    # Size of heatmap
    n_row, n_col = heatmap_values.shape

    if ax is None:
        fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(n_col * 0.2, n_row * 0.2), dpi=150)
    else:
        fig = plt.gcf()

    if cnorm is None:
        cnorm = mpl.colors.Normalize(vmin=np.min(heatmap_values), vmax=np.max(heatmap_values))

    # Alpha and linewidth  mask
    alpha_mask = np.ones_like(heatmap_values, dtype=float)
    lw_mask = 0.5 * np.ones_like(heatmap_values, dtype=float)
    if not (alpha == 'none'):
        alpha_mask = 0.0 * alpha_mask
        np.fill_diagonal(alpha_mask, 1.0)  # diagonal elements have full alpha
        alpha_mask[alpha_mask == 0] = alpha  # off-diagonal elements have reduced alpha
        np.fill_diagonal(lw_mask, 1.0)  # linewidth

    for c in range(n_col):  #
        for r in range(n_row):
            val = heatmap_values[r, c]
            if abs:
                val = np.abs(val)
            color = cmap(cnorm(val))
            ax.add_patch(plt.Rectangle((c + 0.0, n_row - r - 1), 1, 1,
                                       facecolor=color, edgecolor='black',
                                       alpha=alpha_mask[r, c], linewidth=lw_mask[r, c]))

    ax.set_xlim(0, 0 + n_col)
    ax.set_ylim(0, 0 + n_row)
    ax.set_xticks(np.arange(0, 0.0 + n_col) + 0.5)
    ax.set_yticks(np.arange(0, 0.0 + n_row) + 0.5)
    ax.set_aspect(1.0)

    # Set tick labels
    if labels is not None:
        if isinstance(labels, tuple):
            labels_x = list(labels[0])
            labels_y = list(reversed(list(labels[1])))
        else:
            labels_x = list(labels)
            labels_y = list(reversed(list(labels)))

    else:
        labels_x = ['$x_{' + str(n) + '}$' for n in range(n_col)]
        labels_y = ['$y_{' + str(n) + '}$' for n in range(n_row)]
        labels_y.reverse()
    ...
    if right_labels is not None:
        ax_right = ax.twinx()
        ax_right.set_ylim(0, 0 + n_row)
        ax_right.set_yticks(np.arange(0, 0.0 + n_row) + 0.5)
        right_labels.reverse()
        ax_right.set_yticklabels(right_labels, rotation=0)
        ax_right.tick_params(axis='both', which='major', labelsize=6, pad=-2, labelright=True, right=False,
                             bottom=False, labelbottom=False, left=False, labelleft=False, labeltop=False)

    ax.tick_params(axis='both', which='major', labelsize=6, pad=0,
                   bottom=False, labelbottom=False, left=False, labelleft=True, labeltop=True)

    # Colorbar
    # cax = ax.inset_axes((1.2, 0.0, n_col * 0.02, 1.0))
    cax = inset_axes(ax, width='100%', height=0.1, loc='lower left', bbox_to_anchor=(0.0, -1.0 * (1.0 / n_row), 1, 1),
                     bbox_transform=ax.transAxes, borderpad=0)

    cbar = fig.colorbar(mpl.cm.ScalarMappable(norm=cnorm, cmap=cmap), cax=cax,
                        ticks=cmap_ticks, orientation='horizontal')

    cbar.set_label('matrix values', rotation=0, fontsize=7, labelpad=8)
    cbar.ax.tick_params(labelsize=6)
    cbar.ax.minorticks_off()


def plot_density_jitter(y, x_center=0, spread_scale=0.3, scatter_kwargs=None, median_kwargs=None, ax=None, seed=0):

    """
    Plots a jittered scatter plot for a single distribution of y-values.
    Points are centered around `x_center` and spread based on their density.

    :param y: array-like, The y-values of the data points.
    :param x_center: float, The x-coordinate around which points are centered. Default is 0.
    :param spread_scale: float, Controls the horizontal spread of points. Default is 0.3.
    :param scatter_kwargs: dict, Optional dictionary of keyword arguments for `plt.scatter`.
                           Default values:
                           - alpha: 0.6 (transparency)
                           - s: 10 (point size)
                           - color: "black"
    :param median_kwargs: dict, Optional dictionary of keyword arguments for `plt.plot` (median line).
                          Default values:
                          - color: "red"
                          - linewidth: 2
                          - linestyle: "-"
    :param ax: matplotlib.axes.Axes, Optional axis object to plot on. If None, a new figure is created.
    ...
    # Default median line parameters
    default_median_kwargs = {
        'color': 'red',
        'linewidth': 1.0,
        'linestyle': '-'
    }
    ...
    if median_kwargs is not None:
        default_median_kwargs.update(median_kwargs)

    # Compute density using KDE
    kde = stats.gaussian_kde(y)
    densities = kde(y)

    # Normalize densities to control spread
    norm_spread = spread_scale * (densities - np.min(densities)) / (np.max(densities) - np.min(densities) + 1e-6)

    # Jitter x-coordinates based on density
    jitter_x = x_center + norm_spread * (np.random.rand(len(y)) - 0.5)
    ...
    # Plot scatter points
    ax.scatter(jitter_x, y, **default_scatter_kwargs)

    # Plot median line
    ax.plot([x_center - spread_scale / 2, x_center + spread_scale / 2], [median_y, median_y], **default_median_kwargs)
